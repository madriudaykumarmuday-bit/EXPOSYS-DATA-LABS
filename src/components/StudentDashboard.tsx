      ...
      {!application ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Stripe Billing Box */}
            <div className="glass-card rounded-3xl p-6 sm:p-8 border border-white/60 shadow-xl space-y-6 relative overflow-hidden">
              <div className="blob w-[250px] h-[250px] bg-cyan-100 top-[-50px] right-[-50px] opacity-40"></div>
              
              <div className="flex justify-between items-start border-b border-slate-200/50 pb-4 relative z-10">
                <div>
                  <span className="text-[10px] font-bold text-cyan-600 uppercase tracking-widest bg-cyan-50 px-2 py-0.5 rounded-md">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Subscription ID: <code className="bg-slate-50 px-1 py-0.5 rounded text-slate-600 font-mono text-[10px]">{application.subscription?.id || 'sub_stripe_mock'}</code>
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 relative z-10">
                ...
                <div className="p-4 bg-slate-50/70 rounded-2xl border border-slate-100">
                  ...
                  <div className="flex items-center gap-1.5 mt-1 text-slate-700 text-xs font-bold">
                    <span className="text-slate-400">••••</span>
                    <span>{application.subscription?.cardLast4 || '4242'}</span>
                    <span className="text-[10px] uppercase text-indigo-600 px-1 bg-indigo-50 rounded font-black">
              ...
              <div className="bg-slate-50/60 p-4 rounded-2xl gap-3 flex flex-wrap items-center justify-between relative z-10">
                <div className="text-xs text-slate-500 font-medium">
                  {application.subscription?.status === 'active' ? (
                    <span>Automated recurring invoice active via Stripe secure token.</span>
                  ) : (
                    <span className="text-rose-600 font-bold">Stripe recurring charge is currently suspended.</span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  <button
                    ...
                    className="px-4 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-all shadow-sm flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-indigo-55 bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold text-xs rounded-xl transition-all hover:bg-indigo-100"
                  >
                  ...
                  {application.subscription?.status === 'active' ? (
                    <button
                      ...
                      className="px-4 py-2 text-rose-600 hover:text-rose-700 hover:bg-rose-50 font-bold text-xs rounded-xl transition-all"
                    >
                      Pause Subscription
                    </button>
                  ) : (
              ...
              <div className="space-y-3.5 pt-2">
                ...
                <div className="border border-slate-100 rounded-2xl overflow-hidden bg-white/40">
                  <div className="p-3 bg-slate-50 flex justify-between text-[10px] text-slate-400 uppercase font-black">
                  ...
                  <div className="divide-y divide-slate-100">
                    <div className="p-3 flex justify-between items-center text-xs">
                      <div>
                        <p className="font-bold text-slate-700">June 2026 Installment - #INV-001</p>
                        <p className="text-[10px] text-slate-400 mt-0.5">Stripe Gateway (Visa card •••• {application.subscription?.cardLast4 || '4242'})</p>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="font-black text-slate-800">₹{application.subscription?.amount || 999}</span>
                        ...
                        <button 
                          ...
                          className="p-1 px-2 border hover:bg-slate-50 rounded-lg text-slate-400 hover:text-slate-700 transition-colors"
            ...
            <div className="glass-card rounded-3xl p-6 border border-slate-200/50 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-medium text-slate-600">
                <p><strong>Intern Domain:</strong> {application.internshipDomain}</p>
                <p><strong>Duration:</strong> {application.internshipDuration}</p>
                ...
                <p><strong>Uploaded File / CV:</strong> <code className="bg-slate-100 text-slate-750 px-1 rounded truncate">{application.resumeUrl || 'resume.pdf'}</code></p>
              </div>
            </div>
          </div>
